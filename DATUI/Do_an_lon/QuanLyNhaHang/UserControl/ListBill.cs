...
namespace QuanLyNhaHang
{
    public partial class ListBill : UserControl
    {
        ...
        private void cbbTableName_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            dtgvBill.DataSource = MenuDAO.Instance.GetListMenusById(maban);
        ...
        private void dtgvBill_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                txtUnitPrice.Text = row.Cells["GIA"].Value.ToString();
                txtCount.Text = row.Cells["SOLUONG"].Value.ToString();
                txtPrice.Text = row.Cells["THANHTIEN"].Value.ToString();
